...
public class RegistryUtils {
	...
	public static <B extends Block> B register(String name, B block) {
		return register(block, new Identifier(LimberVillMod.ModID, name), ItemGroup.DECORATIONS);
	}

	public static <B extends Block> B register(String name, B block, ItemGroup itemGroup) {
		return register(block, new Identifier(LimberVillMod.ModID, name), itemGroup);
	}
	...
	public static <B extends Block> B registerBlockWithoutItem(String name, B block) {
		if (FabricLoader.getInstance().isDevelopmentEnvironment()) {
			Registry.register(Registry.BLOCK, new Identifier(LimberVillMod.ModID, name), block);
		}
		return block;
	}
	...
	public static <I extends Item> I registerItem(String name, I item) {
		if (FabricLoader.getInstance().isDevelopmentEnvironment()) {
			return Registry.register(Registry.ITEM, new Identifier(LimberVillMod.ModID, name), item);
		}
		return null;
	}
	...
	public static <T extends BlockEntity> BlockEntityType<T> registerBlockEntity(String name, Builder<T> builder) {
		if (FabricLoader.getInstance().isDevelopmentEnvironment()) {
			...
			Registry.register(Registry.BLOCK_ENTITY_TYPE, new Identifier(LimberVillMod.ModID, name), blockEntityType);
	...
	public static Block registerNetherStem(String name, MapColor MapColor) {
		return register(
				...
				new Identifier(LimberVillMod.ModID, name), ItemGroup.BUILDING_BLOCKS);
	}
	...
	public static Block registerLog(String name, MapColor MapColor, MapColor MapColor2) {
		return register(
				...
				new Identifier(LimberVillMod.ModID, name));
	}
